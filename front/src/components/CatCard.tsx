...
export const CatCard: React.FC<ChildProps> = ({cat_id}) => {
    ...
    const addLike = async () => {
        setFavoriteCats([...favoriteCats, cat_id])
    ...
    const removeLike = async () => {
        try {
            const res = await fetch(`/api/likes/${cat_id}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${authToken}`,
                    'Content-Type': 'application/json',
            ...
            if (res.ok) {
                setFavoriteCats(favoriteCats.filter(id => id !== cat_id))
            } else {
    ...
    return (
        <div
            className="card_cat relative h-[225px] w-[225px] bg-black overflow-hidden flex items-center justify-center hover:scale-[calc(257/225)] transition duration-300 ease-in-out cursor-pointer">
            <img className="w-[100%] h-[100%] object-cover"
                 src={`https://cdn2.thecatapi.com/images/${cat_id}.jpg`}/>
            <div
                className="like_container absolute w-[100%] bottom-0 h-[94px] transition duration-300 ease-in-out flex items-center justify-end">
                <div className="w-[48px] h-[48px] mr-[20px] hidden relative like">
                    ...
                    <svg
                        className={`liked_like absolute top-0 left-0 ${favoriteCats.includes(cat_id) ? '' : 'hover:opacity-[1] opacity-[0]'} transition ease-in-out duration-300`}
                        onClick={() => {
                            if (!favoriteCats.includes(cat_id)) {
                                addLike()
                            } else {
                                removeLike()
                            }
                        ...
                        <g clip-path="url(#clip0_1_2180)">
                            <path className={`${favoriteCats.includes(cat_id) ? 'fill-[#FF3A00]' : 'fill-[#F24E1E]'}`}
